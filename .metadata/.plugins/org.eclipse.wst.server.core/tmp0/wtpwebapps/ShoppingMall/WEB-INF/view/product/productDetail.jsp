...
<body>
	...
	<form:hidden path="p_image" value="${product.p_IMAGE}"/>
	</td>
	</tr>
	
	<tr>
	<td colspan="2"><p>${product.p_NAME}</p></td>
	<form:hidden path="p_name" value="${product.p_NAME}"/>

	</tr>
	
	<tr>
	<td colspan="2">${product.p_PRICE}</td>
	<form:hidden path="p_price" value="${product.p_PRICE}"/>
	</tr>
	
	<tr>
	<td>사이즈</td>
	<td>
	<form:select path="str" onchange="select(this.value)">
	<option value="none">=== 선택 ===</option>
	<c:forEach var="std" items="${productStd}" varStatus="status">
	<form:option value="${status.index}">${std.p_color}-${std.p_size}</form:option>
	</c:forEach>
	</form:select>
